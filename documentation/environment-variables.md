...
This is the complete Pallas 0.1.0 environment contract. Copy `.env.example` to
`.env` before `docker compose up --build -d`. Compose stops with a named error
when a required value is absent or empty. FastAPI validates its configuration
before the application starts and reports the offending variable.

Task 4 owns credential hardening. The example credentials here are placeholders,
...
| `POSTGRES_PASSWORD` | **Required** | No default; PostgreSQL password and the password embedded in FastAPI's generated `DATABASE_URL`. |
| `PGADMIN_DEFAULT_EMAIL` | **Required** | No default; syntactically valid email address accepted by PGAdmin. |
| `PGADMIN_DEFAULT_PASSWORD` | **Required** | No default; password accepted by PGAdmin. |

`DATABASE_URL` is generated by Compose and passed to FastAPI; it is not a
user-set `.env` variable in 0.1.0. The API does not yet perform database work.
